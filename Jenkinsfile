        ...
        stage('E2E Testing (Cypress)') {
            steps {
                script {
                    ...
                    sh 'nohup "$JAVA_HOME/bin/java" -jar "$WORKSPACE/target/spring-petclinic-4.0.0-SNAPSHOT.jar" --server.port=8081 >/dev/null 2>&1 &'
            ...
            steps {
                script {
                    ...
                    sh '''
                        ...
                        jmeter -n -t "src/test/jmeter/petclinic_test_plan.jmx" -l "target/jmeter/petclinic_test_plan.jtl"
                        jmeter -n -t "src/test/jmeter/petclinic_improved.jmx" -l "target/jmeter/petclinic_improved.jtl"
                    '''
                }
            }
            post {
